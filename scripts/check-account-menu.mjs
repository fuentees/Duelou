...
try {
  ...
  assert.equal(await page.getByRole("button", { name: "Criar jogador", exact: true }).isDisabled(), true);
  await page.getByLabel("Apelido", { exact: true }).fill("!");
  assert.equal(await page.getByRole("button", { name: "Criar jogador", exact: true }).isDisabled(), true);
  ...
  await page.getByLabel("Código de recuperação", { exact: true }).fill("INVALIDO");
  assert.equal(await page.getByRole("button", { name: "Recuperar conta", exact: true }).isDisabled(), true);
